...
def line_search(
    g: tp.Callable, direction: jnp.ndarray, x0: jnp.ndarray, g0: jnp.ndarray, *args
):
    """
...
def broyden(
    ...
    *args,
    ...
    trace_size: int = 30,
) -> tp.Tuple[jnp.ndarray, BroydenInfo]:
    """
    ...
    def body_fun(state: BroydenState):
        ...
        state = state._replace(
            x=state.x + dx, gx=state.gx + delta_gx, n_step=state.n_step + 1,
        )
        ...
        state = state._replace(
            ...
            prog_break=(new_objective < 3.0 * eps)
            & (state.n_step > trace_size)
            & (jnp.max(state.trace) / jnp.min(state.trace) < 1.3),
        )
...
def broyden_with_vjp(
    ...
    **kwargs,
    ...
    """
    return ad.rootfinder_with_vjp(fun, partial(broyden, **kwargs), jacobian_solver)


def broyden_with_jvp(
    fun: tp.Callable,
    *,
    jacobian_solver: tp.Callable = jax.scipy.sparse.linalg.gmres,
    **kwargs,
    ...
    """
    Get a broyden solver with jvp implemented.
    ...
    """
    return ad.roofinder_with_jvp(fun, partial(broyden, **kwargs), jacobian_solver)
